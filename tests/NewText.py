...
def get_typed(Key, maxlen):
    ...
    global __Shifted,__lastBlink,__blink,__checkfor
    if Key == []: pass
    elif Key == ['|']: Key = []
    elif len(Key) > 0 and Key[-1] == '|': Key = Key[:-1]
    for event in pygame.event.get([pygame.KEYDOWN,pygame.KEYUP]):
        if event.key in [pygame.K_LCTRL,pygame.K_RCTRL] and __checkfor and event.type == pygame.KEYUP:
            __checkfor = False
            continue
        if __checkfor:
            if event.type == pygame.KEYDOWN: pygame.event.post(pygame.event.Event(HOTKEYEVENT,{'value':event.key}))
            continue
        if event.key in [pygame.K_LCTRL,pygame.K_RCTRL] and event.type == pygame.KEYDOWN:
            __checkfor = True
            continue
